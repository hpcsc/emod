...
func (f fakeGitHub) serve(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("/repos/hpcsc/emod/releases/latest", func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("/repos/hpcsc/emod/releases", func(w http.ResponseWriter, r *http.Request) {
		all := []map[string]any{}
		for _, rel := range f.releases {
...
func archiveHolding(t *testing.T, binary string) []byte {
	...
	for name, content := range map[string]string{"README.md": "# emod\n", "emod": binary} {
...
func TestUpdater(t *testing.T) {
	...
	releaseWith := func(t *testing.T, tag string, archives map[string][]byte) *release.Client {
		...
		return release.NewClient(server.Client(), server.URL, "hpcsc/emod", "")
	}
	...
	t.Run("platform", func(t *testing.T) {
		t.Run("names the build the way the release archives do", func(t *testing.T) {
			require.Equal(t, "darwin-arm64", release.Platform("darwin", "arm64"))
			require.Equal(t, "linux-amd64", release.Platform("linux", "amd64"))
		})

		t.Run("spells the 32-bit x86 architecture i386, as the archives do", func(t *testing.T) {
			require.Equal(t, "linux-i386", release.Platform("linux", "386"))
		})
	})
	...
	t.Run("install", func(t *testing.T) {
		t.Run("replaces the executable with the binary from the archive for this platform", func(t *testing.T) {
			client := releaseWith(t, "v0.2.0", map[string][]byte{
				"emod-darwin-arm64.tar.gz": archiveHolding(t, "darwin arm64 binary"),
				"emod-linux-amd64.tar.gz":  archiveHolding(t, "linux amd64 binary"),
			})
		...
		t.Run("reports the bytes of the archive as they arrive, up to the size of the archive", func(t *testing.T) {
			...
			client := releaseWith(t, "v0.2.0", map[string][]byte{"emod-darwin-arm64.tar.gz": archive})
		...
		t.Run("leaves the executable alone when the download does not match its checksum", func(t *testing.T) {
			...
			assets := map[string][]byte{
				"emod-darwin-arm64.tar.gz": archive,
				"checksums.txt":            checksumsFor(map[string][]byte{"emod-darwin-arm64.tar.gz": []byte("something else")}),
			}
			server := fakeGitHub{releases: []fakeRelease{{tag: "v0.2.0", assets: assets}}}.serve(t)
			path := installedBinary(t)
			updater := release.NewUpdater(release.NewClient(server.Client(), server.URL, "hpcsc/emod", ""), "v0.1.0", "darwin-arm64", path)
			check, err := updater.Check(ctx, release.Releases)
			require.NoError(t, err)
		...
		t.Run("leaves the executable alone when the release has no checksums", func(t *testing.T) {
			archive := archiveHolding(t, "new binary")
			server := fakeGitHub{releases: []fakeRelease{{tag: "v0.2.0", assets: map[string][]byte{"emod-darwin-arm64.tar.gz": archive}}}}.serve(t)
			path := installedBinary(t)
			updater := release.NewUpdater(release.NewClient(server.Client(), server.URL, "hpcsc/emod", ""), "v0.1.0", "darwin-arm64", path)
			check, err := updater.Check(ctx, release.Releases)
			require.NoError(t, err)
		...
		t.Run("a release with no archive for this platform names the platform", func(t *testing.T) {
			client := releaseWith(t, "v0.2.0", map[string][]byte{"emod-linux-amd64.tar.gz": archiveHolding(t, "linux amd64 binary")})
			path := installedBinary(t)
			updater := release.NewUpdater(client, "v0.1.0", "darwin-arm64", path)
			check, err := updater.Check(ctx, release.Releases)
			require.NoError(t, err)
