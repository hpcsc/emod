...
var ruleDescriptions = map[string]string{
	...
	"flow/rejection-without-spec": `A rejection edge on the timeline says a command can be refused by an invariant, but nothing in the model shows the scenario in which it is. Write a spec on the same slice whose "when" names that command and whose "then" is a rejection naming that invariant. The search is deliberately slice-local: a spec in another slice describes another slice's behaviour.`,
	...
	"spec/invariant-never-exercised": `An invariant no "then rejected" spec references in its own scope is never exercised — nothing in the model pins it to a scenario where it should hold. Pick a command in the same scope and write a spec whose "when" names that command and whose "then" is a rejection naming this invariant.`,
	"spec/given-outside-boundary": `A spec's "given" names an event the spec's enclosing aggregate does not declare. In aggregate mode the boundary is the aggregate: a spec may only name events its own aggregate's slices declare. In DCB mode the boundary is the "when" command's decides_on: the given events must appear in the list of events the command queries.`,
}
