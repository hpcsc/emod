...
// Format writes the model. hclwrite owns the layout, so the columns line up on
// the `=`.
...
func writeContext(body *hclwrite.Body, context *ast.Context) {
	...
	writeInvariants(body, context.Invariants)
...
func writeAggregate(body *hclwrite.Body, aggregate *ast.Aggregate) {
	...
	writeInvariants(body, aggregate.Invariants)
...
func writeInvariants(body *hclwrite.Body, invariants []*ast.Invariant) {
	if len(invariants) == 0 {
		return
	}
	gap(body)
...
func writeCommand(body *hclwrite.Body, command *ast.Command) {
	...
	writeFields(body, command.Fields)
...
func writeFields(body *hclwrite.Body, fields []*ast.Field) {
	if len(fields) == 0 {
		return
	}
	gap(body)
...
// pinnedVersion is the version the file is written with: the one it declared,
// or the one this tool supports where a model was built rather than read.
