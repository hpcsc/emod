...
`
	...
	t.Run("context reference in automation target context", func(t *testing.T) {
		// the context a target block names is a reference to a declaration
		cLine, cChar := posIn(t, testDoc, "context = Orders", "Orders")
		// A context's name is a quoted label, and its location starts at the
		// name rather than at the quote that opens it.
		dLine, dChar := posIn(t, testDoc, "context \"Orders\"", "Orders")
	...
	t.Run("every construct a spec names resolves to its declaration", func(t *testing.T) {
		t.Run("in a slice an aggregate holds", func(t *testing.T) {
			...
			t.Run("an element of a then event list", func(t *testing.T) {
				cLine, cChar := posIn(t, doc, "[CopyReturned]", "CopyReturned")
		...
		t.Run("in a slice a mode dcb context holds directly", func(t *testing.T) {
			...
			t.Run("an element of a then event list", func(t *testing.T) {
				cLine, cChar := posIn(t, doc, "[DeskReleased]", "DeskReleased")
		...
		t.Run("a then rejected invariant name resolves in the scope that declares it", func(t *testing.T) {
			...
			for _, tc := range []struct {
				...
				t.Run(tc.home, func(t *testing.T) {
					cLine, cChar := posIn(t, doc, "rejected("+tc.invariant, tc.invariant)
					dLine, dChar := posIn(t, doc, tc.invariant+" = ", tc.invariant)
    ...
    aggregate "Loan" {
		...
		t.Run("a cursor on an invariant declaration returns nil", func(t *testing.T) {
			doc := test.SpecLibraryLending
			line, char := posIn(t, doc, "OneCopyPerLoan", "OneCopyPerLoan")
			assertNil(t, doc, line, char)
		})
    ...
    aggregate "Loan" {
        slice "Borrow Copy" {
			...
			line, char := posIn(t, doc, "[CopyReturned]", "CopyReturned")
			assertNil(t, doc, line, char)

			declaredLine, declaredChar := posIn(t, doc, "[CopyBorrowed]", "CopyBorrowed")
			dLine, dChar := posIn(t, doc, `event "CopyBorrowed`, "CopyBorrowed")
	...
	t.Run("both names a flow rejection edge carries resolve to their declarations", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			t.Run(tc.home, func(t *testing.T) {
				...
				invDeclLine, invDeclChar := posIn(t, doc, tc.invariant+" = ", tc.invariant)
