...
func TestVersion(t *testing.T) {
	build := func(moduleVersion, revision, modified string) *debug.BuildInfo {
		info := &debug.BuildInfo{Main: debug.Module{Path: "github.com/hpcsc/emod", Version: moduleVersion}}
		if revision != "" {
	...
	t.Run("from build", func(t *testing.T) {
		...
		t.Run("a build with no build information at all is unknown", func(t *testing.T) {
	...
	t.Run("is tagged", func(t *testing.T) {
		...
		t.Run("an unknown version is not tagged", func(t *testing.T) {
			require.False(t, version.IsTagged(version.Unknown))
		})
	})
}
