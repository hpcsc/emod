...
- [ ] The comments tooltip contains no `input`, `textarea`, `button` or `[contenteditable]` element, and the detail panel for a commented node offers exactly the editable controls the same node without comments offers — the panel's existing field and delete controls stay, and carrying comments adds nothing that writes one
