...
**Acceptance Criteria:**
- [x] `internal/frontend/` holds `static/`, `tests/`, `embed.go`, `package.json`,
      `package-lock.json` and `vitest.config.js`; `internal/viewer/` holds `serve.go` and
      `serve_test.go` and nothing else
- [x] `internal/frontend/embed.go` declares `package frontend` and its `//go:embed` directive reaches
      the same two directories it reaches today (`internal/viewer/embed.go:7`)
- [x] The WASM build output lands beside the assets the same package embeds: `Taskfile.yml:36-38`'s
...
- [x] `internal/viewer/serve.go` reads the embedded assets from the frontend package;
...
- [x] `internal/viewer/serve_test.go` passes with no assertion weakened
- [x] `mise exec -- task build:web` produces `web/index.html`, `web/static/` and `web/generated/`
      with the same file list as before this task
- [x] `test:viewer` and `test:viewer:deps` run in the directory that now holds `package.json`, and
      `mise exec -- task test:viewer` collects all 24 test files and passes
- [x] `.github/workflows/ci.yml:31`'s `node-version-file` names the moved `package.json`
- [x] `docs/architecture.md:213-241` and `docs/wasm-architecture.md:9,19,50,59` name paths that exist
      in the tree after this task
- [x] No file outside `docs/proposals/` and `tasks/` names `internal/viewer/static`,
      `internal/viewer/tests`, `internal/viewer/generated` or `internal/viewer/package.json`
- [x] `mise exec -- task test:e2e:viewer` passes — the web bundle still loads, instantiates the WASM
      module and renders
...
**Acceptance Criteria:**
- [x] `viewer.js` does not name `INITIAL_DATA`; it asks the platform module for the initial state and
      renders it when there is one (today `viewer.js:291-292`)
- [x] `internal/viewer/serve.go` is unchanged: the same `<!--INITIAL_DATA-->` marker at
...
- [x] With no initial state the viewer still opens the data panel, shows the landing instructions,
      sets the name display to `(no model)` and sets the source placeholder —
      `internal/frontend/tests/viewer.test.js:139-150`
- [x] With initial state the viewer still renders the supplied diagram and hides the landing page —
      `:151-162`
- [x] The readiness gate still shows `⏳ Loading parser...` while the platform is not ready and
      clears it once it is — `:163-190`
- [x] `emod diagram --serve <file>` still renders the model on first paint with no Render click
- [x] `mise exec -- task test:viewer`, `test:unit`, `test:integration` and `test:e2e:viewer` pass
