...
- **Question routed to AI when a deterministic query was exact.** Wasted tokens and lower trust. Mitigate by detecting exact-answerable questions and routing them deterministic-first (Phase 5), and documenting that `emod` already answers structural lookups via the LSP and `emod slices` / `emod export`.
